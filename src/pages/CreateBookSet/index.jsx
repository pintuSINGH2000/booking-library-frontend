...
function CreateBookSet() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await bookSetAPI.create(payload);
      setSuccess('Book set created successfully!');
      setTimeout(() => navigate('/book-sets'), 1500);
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to create book set');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <div className="page-header" style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
        ...
        <h1 className="page-title" style={{ margin: 0 }}>Create New Book Set</h1>
      </div>
